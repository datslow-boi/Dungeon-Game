...
class Exits:
    ...
    def update(self):
        ...
        elif find_trigger(self.triggers, "100"):
            self.game.game_state_manager.set_state("win")
            return


    def switch_rooms(self, map):
        # load current room data into list
        temp_data = {}

        temp_data["tiles"] = self.game.world.world_data
        ...
        if map in self.data_list:
            # load room you've been in before
            self.game.world.world_data = self.data_list[map]["tiles"]
